...
export default function FileBrowser({ params }: Props) {
  ...
  useEffect(() => {
    if (resolvedParams.path && resolvedParams.path.length > 0) {
      const [remote, ...pathSegments] = resolvedParams.path;
      setSelectedRemote(decodeURIComponent(remote));
      setCurrentPath(pathSegments.map(segment => decodeURIComponent(segment)).join('/'));
    }
  }, [resolvedParams.path]);
  ...
  const handleRemoteSelect = (remote: string) => {
    ...
    window.history.pushState(null, '', `/${encodeURIComponent(remote)}`);
  ...
  const handleFileClick = (file: FileItem, event: React.MouseEvent) => {
    ...
    if (file.IsDir) {
      ...
      const newUrl = selectedRemote ? 
        `/${encodeURIComponent(selectedRemote)}/${newPath.split('/').map(p => encodeURIComponent(p)).join('/')}` :
        '/browser';
  ...
  const handleBreadcrumbNavigate = (path: string) => {
    const newUrl = selectedRemote ? 
      `/${encodeURIComponent(selectedRemote)}${path ? '/' + path.split('/').map(p => encodeURIComponent(p)).join('/') : ''}` :
      '/browser';
    window.history.pushState(null, '', newUrl);
    setCurrentPath(path);
  };
